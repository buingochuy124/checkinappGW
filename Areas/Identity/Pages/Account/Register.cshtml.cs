...
namespace CheckInGWDN.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    if (role == SD.Manager && (User.IsInRole(SD.Administrator) || User.IsInRole(SD.Manager)))
                    {
                        await _userManager.AddToRoleAsync(user, SD.Manager);
                    }
                    else
                    {
                        if (role == SD.Staff && (User.IsInRole(SD.Administrator) || User.IsInRole(SD.Manager)))
                        {
                            await _userManager.AddToRoleAsync(user, SD.Staff);
                        }
                        else
                        {
                            if (role == SD.Administrator && User.IsInRole(SD.Administrator))
                            {
